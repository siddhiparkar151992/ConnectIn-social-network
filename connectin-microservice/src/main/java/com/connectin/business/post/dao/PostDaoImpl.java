...
public class PostDaoImpl implements IPostDao {
        ...
        try {
            posts = (List<PostDTO>) entityManager
                    .createQuery("select new com.connectin.domain.post.PostDTO(p.id, p.category.categoryName, "
                            ...
                            + "from post p join  p.user.profileImage i where p.user.userName=:userName order by p.createdTime desc")
    ...
    public List<PostDTO> getPostsByFeed(List<String> connections) throws ConnectinBaseException {
        ...
        try {
            posts = (List<PostDTO>) entityManager
                    .createQuery("select new com.connectin.domain.post.PostDTO(p.id, p.category.categoryName, "
                            + "p.visibility, p.tags, p.createdTime,p.text, u, p.user.profileImage.type,p.user.profileImage.url, p.user.profileImage.alt) "
                            + "from post p join p.user u " +
                            "where p.user.userName in (:users) " +
